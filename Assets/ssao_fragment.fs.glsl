...
void main()
{
    // Use depth_map(proj space) to get the depth
    float depth = texture(depth_map, fs_in.texcoord).r;
    if(depth == 1.0) { discard; } // Why this line?
    
    // position is the current pixel in view space
    mat4 invproj = inverse(proj);
    vec4 position = invproj * vec4(vec3(fs_in.texcoord, depth) * 2.0 - 1.0, 1.0);
    position /= position.w;
    
    // use normal_map(proj space) to get the normal vector
